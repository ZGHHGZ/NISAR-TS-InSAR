...
def run(cfg, resample_type):
    ...
    for freq in freq_pols.keys():
        ...
        resamp_obj = Resamp(radar_grid,native_doppler,ref_rdr_grid=ref_radar_grid)
        ...
        for pol in pol_list:
            ...
            if resample_type == 'coarse':
                print("___________________coarse__________________________")
                resamp_obj.resamp(input_raster, resamp_slc, rg_off, az_off,1,True)

            else:
                resample_slc_scratch_path = scratch_path / 'coarse_resample_slc' / f'freq{freq}'
                out_dir = resample_slc_scratch_path / pol
                raster_path = str(out_dir/'coregistered_secondary.slc')
                input_raster = isce3.io.Raster(raster_path)
                rg_off = isce3.io.Raster(str(offsets_path / 'resampled_rg_offsets'))
                az_off = isce3.io.Raster(str(offsets_path / 'resampled_az_offsets'))
                print("___________________fine__________________________")
